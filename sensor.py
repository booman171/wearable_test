...
class Sensor:
    ...
    def read_from_port(self):
        ...
        while True:
            ...
            if self.ecg < thresh and N > (IBI/5.0)*3:     # avoid dichrotic noise by waiting 3/5 of last IBI
                if self.ecg < T:                          # T is the trough
                    T = self.signal                          # keep track of lowest point in pulse wave 

            if self.ecg > thresh and self.ecg > P:
                P = self.ecg

            # signal surges up in value every time there is a pulse
            if N > 250:                                 # avoid high frequency noise
                if self.ecg > thresh and Pulse == False and N > (IBI/5.0)*3:
